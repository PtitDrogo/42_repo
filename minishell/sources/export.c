...
/*   Updated: 2024/04/10 18:13:58 by tfreydie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	export(t_env_node **root, void *variable);
...
int main(int argc, char const *argv[], char **envp)
{
	...
	while (envp[++i]) // add condition if export fails
		export(&env_dup_root, (void *)envp[i]);
	//test
	unset(env_dup_root, "DISPLAY=");
	unset(env_dup_root, "IdontExist");
	export(&env_dup_root, "MIAOU=VALGRIND");
	export(&env_dup_root, "MIAOU2=VALGRIND2");
	export(&env_dup_root, "MIAOU3=VALGRIND3");
